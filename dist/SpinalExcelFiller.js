...
function cloneWorksheetContent(src, dst) {
...
export class SpinalExcelFiller {
    ...
    addSheet(name, options = {}) {
        ...
        if (options.copyFrom !== undefined) {
            ...
            if (!source) {
                throw new Error(`Source worksheet "${options.copyFrom}" not found in the template.`);
            }
     ...
     */
    async toBufferWithCharts(originalBuffer) {
        if (!this.workbook) {
            throw new Error("No template loaded. Call loadTemplate() first.");
        }
        ...
        const chartRelatedFiles = Object.keys(origZip.files).filter((name) => /^xl\/(charts|drawings)\//.test(name) ||
            /^xl\/worksheets\/_rels\//.test(name));
        for (const path of chartRelatedFiles) {
            const f = origZip.file(path);
            if (f)
                filledZip.file(path, await f.async("uint8array"));
        }
        ...
        const worksheetPaths = Object.keys(origZip.files).filter((name) => /^xl\/worksheets\/sheet\d+\.xml$/.test(name));
        ...
        if (filledCTFile && origCTFile) {
            ...
            const missingOverrides = (origCT.match(/<Override[^>]*\/>/g) ?? []).filter((ov) => /drawing|chart/i.test(ov) && !filledCT.includes(ov));
            if (missingOverrides.length > 0) {
                filledZip.file("[Content_Types].xml", filledCT.replace("</Types>", missingOverrides.join("") + "</Types>"));
            }
        }
